...
void test_net_pkt_headroom(void)
{
	...
	/* 1st fragment has 1 byte headroom and one byte available: "H-" */
	...
	/* 3rd fragment has 1 byte headroom and one byte available: "H-" */
	...
	/* 4th fragment has no headroom and two available bytes: "--" */
	...
	zassert_equal(frag1->size, 1, "Size mismatch");
	zassert_equal(frag1->len, 1, "Length mismatch");
	zassert_equal(frag2->size, 0, "Size mismatch");
	zassert_equal(frag2->len, 0, "Length mismatch");
	zassert_equal(frag3->size, 1, "Size mismatch");
	...
	net_pkt_cursor_init(pkt);
	zassert_true(net_pkt_read(pkt, data, 4) == 0, "Pkt read failed");
	zassert_mem_equal(data, "1234", 4, "Data mismatch");
	...
	net_pkt_cursor_init(pkt);
	zassert_true(net_pkt_read(pkt, data, 8) == 0, "Pkt read failed");
	zassert_mem_equal(data, "A1BCD234", 8, "Data mismatch");
