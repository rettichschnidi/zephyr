...
const static uint8_t key[16] __aligned(32) = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
					      0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};

static uint8_t plaintext[64] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e,
...
int ecb_mode(const struct device *dev)
{
	int ret;
	...
	uint8_t ecb_plaintext[16] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
				     0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
	uint8_t ecb_ciphertext[16] = {0x69, 0xC4, 0xE0, 0xD8, 0x6A, 0x7B, 0x04, 0x30,
...
int cbc_mode(const struct device *dev)
{
	int ret;
	...
	ret = cipher_begin_session(dev, &ini, CRYPTO_CIPHER_ALGO_AES, CRYPTO_CIPHER_MODE_CBC,
				   CRYPTO_CIPHER_OP_ENCRYPT);
	if (ret) {
		return ret;
	}

	ret = cipher_cbc_op(&ini, &encrypt, iv);
	if (ret) {
	...
	ret = cipher_cbc_op(&ini, &decrypt, encrypted);
	if (ret) {
...
int ctr_mode(const struct device *dev)
{
	int ret;
	...
	ret = cipher_begin_session(dev, &ini, CRYPTO_CIPHER_ALGO_AES, CRYPTO_CIPHER_MODE_CTR,
				   CRYPTO_CIPHER_OP_ENCRYPT);
	if (ret) {
		return ret;
	}

	ret = cipher_ctr_op(&ini, &encrypt, iv);
	if (ret) {
...
static uint8_t ccm_data[23] = {0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
...
int ccm_mode(const struct device *dev)
{
	int ret;
	...
	ret = cipher_begin_session(dev, &ini, CRYPTO_CIPHER_ALGO_AES, CRYPTO_CIPHER_MODE_CCM,
				   CRYPTO_CIPHER_OP_ENCRYPT);
	if (ret) {
		return ret;
	}
...
static uint8_t gcm_data[42] = {0x08, 0x00, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
...
int gcm_mode(const struct device *dev)
{
	int ret;
	...
	ret = cipher_begin_session(dev, &ini, CRYPTO_CIPHER_ALGO_AES, CRYPTO_CIPHER_MODE_GCM,
				   CRYPTO_CIPHER_OP_ENCRYPT);
	if (ret) {
		return ret;
	}
	...
	gcm_op.pkt = &encrypt;
	ret = cipher_gcm_op(&ini, &gcm_op, gcm_nonce);
	if (ret) {
...
struct mode_test {
	const char *mode;
	/* Returns on success, negative value on error */
	int (*mode_func)(const struct device *dev);
};
...
int main(void)
{
	...
	for (i = 0; modes[i].mode; i++) {
		...
		ret = modes[i].mode_func(dev);
		end_time = timing_counter_get();

		total_cycles = timing_cycles_get(&start_time, &end_time);
		total_ns = timing_cycles_to_ns(total_cycles);
		printk("%s %s after %" PRIu64 " ns, %" PRIu64 " cycles\n", modes[i].mode,
		       ret == 0 ? "succeeded" : "failed", total_ns, total_cycles);
	}
