...
static int crypto_si32_dma_setup_tx(struct cipher_pkt *pkt, uint_fast8_t in_buf_offset)
{
	...
	if (pkt->out_buf_max < pkt->in_len - in_buf_offset) {
...
static int crypto_si32_dma_setup_rx(struct cipher_pkt *pkt, uint_fast8_t in_buf_offset,
				  ...
				  const uint8_t iv[16])
{
	...
	LOG_INF("out_buf_offset=%d, in_buf_offset=%d", out_buf_offset, in_buf_offset);
